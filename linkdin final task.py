...
try:
    ...
    for index, row in profiles_df.iterrows():
        ...
        #     print(f"An error occurred during the process: {e}")
            ...
            #----------------------------combine experience-------------------------#
            # try:
            #     experience_sections = driver.find_elements(By.XPATH,"//section[contains(@class, 'experience')]//ul[contains(@class, 'experience__list')]/li")
            #     experiences = []
            #
            #     for section in experience_sections:
            #         # Fetch all roles within a company
            #         roles = section.find_elements(By.XPATH, ".//div[contains(@class, 'pl-0.5 grow break-words')]")
            #
            #         # If multiple roles exist, iterate through them
            #         if roles:
            ...
            #                 experiences.append(f"{job_title} at {company_name} ({date_range}, {location})")
            #         else:
            #             # Standard single-role structure
            ...
            #
            # except Exception as e:
            #     profile_data['Experience'] = f"Experience not found. Error: {str(e)}"
            ...
            try:
                ...
                for section in experience_sections:
                    try:
                        # Check if multiple roles exist in a section
                        roles = section.find_elements(By.XPATH, ".//div[contains(@class, 'pl-0.5 grow break-words')]")

                        if roles:
                            # Get company name once for all roles
                            company_name = section.find_element(By.XPATH,
                                                                ".//span[@class='experience-item__subtitle']").text
                            for role in roles:
                                try:
                                    job_title = role.find_element(By.XPATH,
                                                                  ".//span[@class='experience-item__title']").text
                                    date_range = role.find_element(By.XPATH,
                                                                   ".//span[contains(@class, 'date-range')]").text
                                    location_elements = role.find_elements(By.XPATH,
                                                                           ".//p[contains(@class, 'experience-item__meta-item')]")
                                    location = location_elements[-1].text if location_elements else "N/A"
                                    experiences.append(f"{job_title} at {company_name} ({date_range}, {location})")
                            ...
                            try:
                                job_title = section.find_element(By.XPATH,
                                                                 ".//span[@class='experience-item__title']").text
                                company_name = section.find_element(By.XPATH,
                                                                    ".//span[@class='experience-item__subtitle']").text
                                date_range = section.find_element(By.XPATH,
                                                                  ".//span[contains(@class, 'date-range')]").text
                                location_elements = section.find_elements(By.XPATH,
                                                                          ".//p[contains(@class, 'experience-item__meta-item')]")
                                location = location_elements[-1].text if location_elements else "N/A"
